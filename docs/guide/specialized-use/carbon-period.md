...
By default, in PHP, when adding "1 month", it does +1 to the month component of the date
and if the obtained date does not exist, it will "overflow":

```php
$period = CarbonPeriod::create('2026-01-31', '1 month', 4);
// Or $period = \DatePeriod::createFromISO8601String('R3/2026-01-31T00:00:00Z/P1M')
$dates = [];
foreach ($period as $date) {
    $dates[] = $date->format('m-d');
}
...
It works like normal period (either with and end date or a
number of recurrences, and can have options set), on the top
of that you can set a different `anchorDay` or use the
`mode: OverflowMode::NoOverflow`.

This mode works like below:
```php
$period = CarbonPeriod::monthly('2026-01-31', recurrences: 4, mode: OverflowMode::NoOverflow);
$dates = [];
foreach ($period as $date) {
    $dates[] = $date->format('m-d');
}
...
`anchorDay` allows to set the reference day from a date
that is not in the period:

```php
$period = CarbonPeriod::monthly('2026-02-28', '2026-05-31', anchorDay: 31, options: CarbonPeriod::EXCLUDE_END_DATE);
$dates = [];
foreach ($period as $date) {
    $dates[] = $date->format('m-d');
}
...
```php
$period = CarbonPeriodImmutable
    ::every('3 days 12 hours')
    ->since('2000-01-01 12:00')
    ->until('2000-01-15 12:00');
...
// You can also change the date class after creation:
...
### Compare periods and check dates in
